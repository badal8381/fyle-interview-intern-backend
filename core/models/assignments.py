...
class Assignment(db.Model):
    ...
    def upsert(cls, assignment_new: 'Assignment'):
        if assignment_new.id is not None:
            ...
            assertions.assert_valid(assignment.content is not None, 'Content should not be none')
    ...
    def submit(cls, _id, teacher_id, auth_principal: AuthPrincipal):
        ...
        if assignment.state == AssignmentStateEnum.DRAFT:
            ...
            return {'error': 'FyleError', 'message': 'only a draft assignment can be submitted'}
    ...
    def mark_grade_by_principal(cls, _id, grade, auth_principal: AuthPrincipal):
        assignment = Assignment.get_by_id(_id)
        assertions.assert_found(assignment, 'No assignment with this id was found')
        assertions.assert_valid(assignment.state != AssignmentStateEnum.DRAFT, 'only assignment in draft state can be graded')
        assertions.assert_valid(grade is not None, 'assignment with empty grade cannot be graded')
